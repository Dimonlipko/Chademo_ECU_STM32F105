...
//constexpr static uint32_t CAN2Base = CAN2_BASE;
...
static bool setupCANpin(int);

class meCAN1
{
private:
  uint32_t contributed_BTR;
  uint32_t interrupt_prioity;
...
public:
  ...
  bool getSilentMode() { return CAN1->BTR >> 31; }
  void setAutoTxRetry(bool val = true) { MODIFY_BIT(CAN1->MCR, 4, !val); } 
   
  void setSilentMode(bool val) { MODIFY_BIT(CAN1->MCR, 4, val); }//periphBit(btr1, 31)
  ...
  void overRideBTR(uint32_t newBTR){contributed_BTR = newBTR;}
  volatile int rxMsgLen = -1; // CAN parms
  volatile int id, fltIdx;
  volatile MSG rxData;  
  meCAN1(){contributed_BTR=0;interrupt_prioity=2;}
  ~meCAN1() {}
};
...
protected:
public:
  bool begin(int brp = 125, int Rx=PB12,int Tx=PB13,idtype addrType = STD_ID_LEN);
  void enableInterrupt(void);
  void disableInterrupt(void);
  void filterList16Init(int bank, int idA = 0, int idB = 0, int idC = 0, int idD = 0);
  void filterList32Init(int bank, int idA = 0, int idB = 0); 
  ...
  volatile int id, fltIdx;
  volatile MSG rxData;  
  ~meCAN2() {contributed_BTR=0;interrupt_prioity=2UL;}
};

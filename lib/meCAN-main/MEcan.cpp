...
bool meCAN1::transmit(int txId, const void *ptr, unsigned int len)
{
    ...
    if (_extIDs)
        *_TIOR = (txId << 3) + 0b100; // set 29-bit extended ID
    else
        *_TIOR = (txId << 21) + 0b000; // set 12-bit standard ID

    *_TDT0R = len;

    // Clear *_TDL0R and *_TDH0R before setting values
    *_TDL0R = 0;
    *_TDH0R = 0;

    // Copy data from ptr according to len
    const uint8_t *data = (const uint8_t *)ptr;
    for (unsigned int i = 0; i < len; i++) {
        if (i < 4) {
            *_TDL0R |= data[i] << (i * 8);  // Fill lower register
        } else {
            *_TDH0R |= data[i] << ((i - 4) * 8);  // Fill upper register
        }
    }

    *_TIOR |= (1 << 0); // request transmission
#endif
    return true;
}
...
bool meCAN2::transmit(int txId, const void *ptr, unsigned int len)
{
    ...
    if (!(tsr_reg & (1 << 26))) // Поштовховий буфер 0 не готовий
    {
        return false;
    }
    if (_extIDs)
        *_TIOR = (txId << 3) + 0b100; // встановлення 29-бітного розширеного ідентифікатора
    else
        *_TIOR = (txId << 21) + 0b000; // встановлення 12-бітного стандартного ідентифікатора

    *_TDT0R = len;

    // Очищення регістрів *_TDL0R і *_TDH0R
    *_TDL0R = 0;
    *_TDH0R = 0;

    // Копіювання даних із ptr згідно з len
    const uint8_t *data = (const uint8_t *)ptr;
    for (unsigned int i = 0; i < len; i++) {
        if (i < 4) {
            *_TDL0R |= data[i] << (i * 8);  // Заповнення нижнього регістра
        } else {
            *_TDH0R |= data[i] << ((i - 4) * 8);  // Заповнення верхнього регістра
        }
    }

    *_TIOR |= (1 << 0); // запит на передачу
#endif
    return true;
}
